...
const RIGTH_SIDEBAR_WIDTH = "18rem";
...
function SidebarProvider({
  ...
  defaultOpen?: boolean | { left?: boolean; right?: boolean };
  open?: boolean | { left?: boolean; right?: boolean };
          ...
          style={
            {
              ...
              "--right-sidebar-width": RIGTH_SIDEBAR_WIDTH,
...
function Sidebar({
  ...
  const sidebar = side === "left" ? left : right;
...
function SidebarTrigger({
  ...
  return (
    <Button
      ...
      onClick={(event) => {
        onClick?.(event);
        toggleSidebar(side); // Pass the side to the toggle function
      }}
      {...props}
    >
...
function SidebarInset({ className, ...props }: React.ComponentProps<"main">) {
  return (
    <main
      ...
      className={cn(
        "bg-background relative flex w-full flex-1 flex-col p-0 m-0",
        className
      )}
      {...props}
    />
...
function SidebarGroupLabel({
  ...
  return (
    <Comp
      ...
      className={cn(
        "ring-sidebar-ring flex h-8 shrink-0 items-center rounded-md px-2 text-sm font-medium outline-hidden transition-[margin,opacity] duration-200 ease-linear focus-visible:ring-2 [&>svg]:size-4 [&>svg]:shrink-0",
...
function SidebarMenuButton({
  ...
  const isMobile = left.isMobile;
  const state = left.state;
